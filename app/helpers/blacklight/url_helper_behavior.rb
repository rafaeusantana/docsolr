...
module Blacklight::UrlHelperBehavior
  ...
  def link_to_document(doc, opts={:label=>nil, :counter => nil})
    ...
    link_to label, "/"+doc.id, document_link_params(doc, opts)
  end
  ...
  def remove_date_range(p, field, value)
    if field=="data" && value=="date_range"
      p[:date_range] = (p[:date_range] || "").dup
      p.delete(:date_range)
    end
    p
  end
  ...
  def remove_facet_params(field, item, source_params=params)
    ...
    p = reset_search_params(source_params)
    p = remove_date_range(p, field, value)
